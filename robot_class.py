...
class robot:
    ...
    def sense(self):
        ...
        ## Iterate through all of the landmarks in a world for each landmark
        for landmarks, (landmark_x, landmark_y) in enumerate(self.landmarks):
        ## Compute dx and dy, the distances between the robot and the landmark
            dx = landmark_x - self.x
            dy = landmark_y - self.y
        ## Account for measurement noise by *adding* a noise component 
            Q = self.rand() * self.measurement_noise
            dx, dy = dx + Q, dy + Q
        ## 3. If either of the distances, dx or dy, fall outside of the internal var, measurement_range
        ##    then we cannot record them; if they do fall in the range, then add them to the measurements list
        ##    as list.append([index, dx, dy]), this format is important for data creation done later
            if dx < self.measurement_range and dy < self.measurement_range:
                measurements.append((landmarks, dx, dy))
